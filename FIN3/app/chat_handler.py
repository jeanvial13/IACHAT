...
def generate_charts(dems):
    ...
    # Premium colors: Red, Amber, Blue, Emerald
    colors_prio = ['#ef4444', '#f59e0b', '#3b82f6', '#10b981']
    
    fig1, ax1 = plt.subplots(figsize=(5, 4))
    wedges, texts, autotexts = ax1.pie(
        sizes, labels=labels, autopct='%1.1f%%', startangle=90, 
        colors=colors_prio, wedgeprops={'width': 0.4, 'edgecolor': 'w'}
    )
    plt.setp(autotexts, size=8, weight="bold", color="white")
    plt.setp(texts, size=9)
    ax1.axis('equal')
    plt.title("Priority Distribution", pad=20)
    
    buf1 = io.BytesIO()
    plt.savefig(buf1, format='png', transparent=True, dpi=150)
    buf1.seek(0)
    ...
    bars = ax2.bar(sla_counts.keys(), sla_counts.values(), color=['#10b981', '#ef4444'], width=0.5)
    plt.title("SLA Status", pad=20)
    
    buf2 = io.BytesIO()
    plt.savefig(buf2, format='png', transparent=True, dpi=150)
    buf2.seek(0)
    charts['sla'] = buf2
    plt.close(fig2)
    ...
    colors_status = ['#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#f59e0b', '#10b981', '#6366f1', '#14b8a6']
    
    wedges3, texts3, autotexts3 = ax3.pie(
        status_counts.values(), labels=status_counts.keys(), autopct='%1.1f%%', startangle=140, 
        colors=colors_status[:len(status_counts)], wedgeprops={'width': 0.4, 'edgecolor': 'w'}
    )
    plt.setp(autotexts3, size=8, weight="bold", color="white")
    plt.setp(texts3, size=8)
    ax3.axis('equal')
    plt.title("Project Status", pad=20)

    buf3 = io.BytesIO()
    plt.savefig(buf3, format='png', transparent=True, dpi=150)
    buf3.seek(0)
    charts['status'] = buf3
    plt.close(fig3)
    ...
    if fmt == "pdf":
        ...
        for d in dems:
            ...
            table_data.append([
                ...
                Paragraph(full_comment, styles["Normal"])
            ])
