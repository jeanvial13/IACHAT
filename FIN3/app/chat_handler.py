...
def build_portfolio_text(dems):
    ...
    for e in enriched:
        ...
        # AI Comment
        ai_comment = generate_ai_comment(e)
        lines.append(f"AI Insight: {ai_comment}")
...
def generate_amd_ai_report_logic(projects):
    """
    Generates a strategic advice report for the given projects using OpenAI.
    Focuses on SAP 4 HANA, Cloud, and general IT best practices.
    """
    if not projects:
        return "No active projects to analyze."

    # Build a summary string of the projects
    projects_summary = ""
    for p in projects:
        name = p.get("name", "Unnamed")
        status = p.get("status", "Unknown")
        notes = p.get("notes", [])
        last_note = notes[-1].get("text", "") if notes else "No notes"
        projects_summary += f"- Project: {name} | Status: {status} | Last Note: {last_note}\n"

    prompt = (
        "You are a senior IT Strategic Advisor specializing in SAP S/4HANA, Cloud Migrations, and Enterprise Architecture.\n"
        "Review the following list of active projects and provide a high-level strategic report.\n"
        "For each project, provide a brief, actionable tip or insight related to SAP S/4HANA best practices, risk mitigation, or acceleration.\n"
        "If a project seems stuck, suggest a way to unblock it.\n\n"
        "Projects List:\n"
        f"{projects_summary}\n\n"
        "Format the output as a professional executive summary with a section for each project.\n"
        "Use a professional, slightly technical tone."
    )
