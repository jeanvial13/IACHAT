...
def build_portfolio_html(projects):
    """Generates a Minimalist Cyberpunk HTML report for the portfolio."""
    now_str = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
    
    html = f"""
    <div style="font-family: 'Rajdhani', sans-serif; color: #e0e0e0; max-width: 1200px; margin: 0 auto;">
        <!-- HEADER -->
        <div style="border-bottom: 1px solid #333; padding-bottom: 20px; margin-bottom: 40px; display: flex; justify-content: space-between; align-items: flex-end;">
            <div>
                <h1 style="color: #fff; font-size: 24px; margin: 0; font-weight: 600; letter-spacing: 1px;">
                    ANDRES VILLANUEVA <span style="color: #00f3ff;">//</span> PROJECT STATUS
                </h1>
            </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 60px;">
            <div style="padding: 10px 0;">
                <div style="font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">Active Projects</div>
                <div style="font-size: 32px; color: #fff; font-weight: 500;">{len(projects)}</div>
            </div>
            <div style="padding: 10px 0;">
                <div style="font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">Critical (P1)</div>
                <div style="font-size: 32px; color: #ff2a2a; font-weight: 500;">
                    {len([p for p in projects if str(p.get('priority')) == '1'])}
                </div>
            </div>
            <div style="padding: 10px 0;">
                <div style="font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">SLA Breached</div>
                <div style="font-size: 32px; color: #fcee0a; font-weight: 500;">
                    {len([p for p in projects if p.get('sla_breached')])}
                </div>
            </div>
            <div style="padding: 10px 0;">
                <div style="font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">Completed</div>
                <div style="font-size: 32px; color: #00f3ff; font-weight: 500;">
        ...
        <div style="margin-bottom: 20px;">
            <h2 style="font-size: 14px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 20px; font-weight: 400;">
                Active Workstreams
            </h2>
    """
    ...
    for p in projects:
        ...
        sponsor = p.get("sponsor", "N/A")
        requester = p.get("requester", "N/A")
        cost_center = p.get("cost_center", "N/A")
        ba_owner = p.get("ba_owner", "N/A")
        notes = p.get("notes", [])
        
        # Get last AI comment if available, else last note
        ai_comment = ""
        if notes:
            # Check for AI comment first
            for n in reversed(notes):
                if "AI Insight" in n.get("text", ""):
                    ai_comment = n.get("text")
                    break
            
            if not ai_comment:
                ai_comment = notes[-1]["text"]
        else:
            ai_comment = "No updates available."
        ...
        html += f"""
        <div style="display: grid; grid-template-columns: 250px 1fr; gap: 40px; padding: 25px 0; border-top: 1px solid #222;">
            <!-- Left Col: Meta -->
            <div>
                <div style="color: {priority_color}; font-size: 11px; font-family: 'Roboto Mono', monospace; margin-bottom: 8px;">
                    {p_id} // P{priority}
                </div>
                <div style="color: #fff; font-size: 16px; font-weight: 500; margin-bottom: 10px; line-height: 1.4;">
                    {name}
                </div>
                ...
                <!-- Additional Details -->
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-family: 'Roboto Mono', monospace; font-size: 10px; color: #555;">
                    <div>
                        <div style="color: #444; margin-bottom: 2px;">SPONSOR</div>
                        <div style="color: #888;">{sponsor}</div>
                    </div>
                    <div>
                        <div style="color: #444; margin-bottom: 2px;">REQUESTER</div>
                        <div style="color: #888;">{requester}</div>
                    </div>
                    <div>
                        <div style="color: #444; margin-bottom: 2px;">COST CENTER</div>
                        <div style="color: #888;">{cost_center}</div>
                    </div>
                    <div>
                        <div style="color: #444; margin-bottom: 2px;">BA OWNER</div>
                        <div style="color: #888;">{ba_owner}</div>
                    </div>
                </div>
            </div>

            <!-- Right Col: Status & Insight -->
            <div>
                <div style="display: flex; gap: 20px; margin-bottom: 15px; font-family: 'Roboto Mono', monospace; font-size: 11px; color: #888;">
                    <div><span style="color: #444;">STATUS:</span> <span style="color: #ccc;">{status}</span></div>
                    <div><span style="color: #444;">WORKFLOW:</span> <span style="color: #ccc;">{workflow}</span></div>
                </div>
                <div style="font-family: 'Roboto Mono', monospace; font-size: 12px; color: #aaa; line-height: 1.6; background: #111; padding: 15px; border-left: 2px solid {priority_color};">
                    {ai_comment}
                </div>
            </div>
        </div>
    ...
    html += """
        <div style="margin-top: 60px; border-top: 1px solid #333; padding-top: 20px; text-align: center; color: #444; font-size: 10px; font-family: 'Roboto Mono', monospace;">
            END OF LINE
        </div>
    </div>
    """
